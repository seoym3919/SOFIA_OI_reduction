"""
 ...
 This script makes Level 2 TP observation data by applying beam efficiency.  
...
dir_TP_L1c = './TA_TP_L1c/'


# Create directory for Level 2 data
os.system('rm -rf TA_TP_L2')
os.system('mkdir TA_TP_L2')
dir_out = './TA_TP_L2/'

# Read Level 1c data
file = glob.glob(dir_TP_L1c+'*.fits')

# Get the file names
file_names = []
for i0 in range(len(file)):
	file_names.append(file[i0].split(dir_TP_L1c)[1])
...
#	
	...
	for i0 in range(0, NFILE_BEAM):
		...
		spec = (hdu[0].data).copy()    # get TP spectra and copy to new array
		...
		file_TP_L2 = dir_out+FNAME_BEAM[i0]   # get L2 file name 
		hdu_out = fits.PrimaryHDU(spec[:]/beam_eff_single, header = hdu[0].header)   # apply beam efficiency 
		hdu_out.writeto(file_TP_L2)		# save L2 spectra
		print('Saving beam efficiency corrected data. File name = '+ file_TP_L2)
	
		spec_total[:] = spec_total[:] + (spec[:]/beam_eff_single) # accumulate spectra for plotting later
	print('\n')
#

# generate sample plot of TP spectrum averaged over the entire region
FIG_NAME = './averaged_TP_total.png'
fig = plt.figure(figsize=(8,6))
vel = (np.arange(NCHAN)-(hdu[0].header['CREFPIX']-1))*hdu[0].header['VELRES']+hdu[0].header['CREFVAL']

plt.plot(vel, spec_total/np.float64(NFILE_BEAM*np.float64(len(beam2process))), drawstyle='steps')
plt.xlim([-10.,30])
plt.ylim([-1.,8])
